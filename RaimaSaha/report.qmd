...
ggplot() + geom_sf(data = my_sf, aes(color = hectare)) + labs(title = "Hectare against latitudinal and longitudinal coordinates", color = "Hectare") + theme(axis.text.x = element_text(angle = 90), plot.title = element_text(hjust = -4))
```
...
fig <= fig |> add_trace(data = temp_data |> filter(human_interaction_num == 1), name = "Approaches")
fig <= fig |> add_trace(data = temp_data |>filter(human_interaction_num == 2), name = "Indifferent")
fig <= fig |> add_trace(data = temp_data |>filter(human_interaction_num == 3), name = "Runs from")
fig
```
...
Figure 6 is a biplot of the PCA. `latitude` and `longitude` appear to be closely correlated, which is consistent with the previous findings. The near 90 degree angle by `shift_num`, `location_num`, and `latitude` and `longitude` vectors show that these variables are not closely correlated.
...
Figure 8 visualizes the top 3 principal components in 3D space. Again, the points are colored based on Human Interaction, with 1 = Approaches, 2 = Indifferent, and 3 = Runs from. All the points run quite uniform in specific linear clusters when looking at the plot from different angles. \*fix figure numbering
